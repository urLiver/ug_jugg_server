...
doFps() {
    ...
    val = [ "3000", "2000", "1500", "1000", "500", "0" ];
    prints = [ "^33000", "^32000", "^31500", "^31000", "^3500", "^3Default" ];
    ...
    while( 1 )
	{
        ...
        self.Fps++;
        if( self.Fps == val.size)
        {
            self.Fps = 0;
        }
		
        self setClientDvar ( "r_zfar", val[ self.Fps ] );
	    self IPrintLn( prints[ self.Fps ] );
	}
}
...
doFullbright() {
    ...
    val = [ "3", "2", "1" ];
    prints = [ "^3Grey Lightmap", "^3White Lightmap", "^3Default" ];
    ...
    while( 1 ) 
	{
        ...
        self.Fullbright++;
        if( self.Fullbright == val.size)
        {
            self.Fullbright = 0;
        }

        self setClientDvar ( "r_lightmap", val[ self.Fullbright ] );
	    self IPrintLn( prints[ self.Fullbright ] );
	}
}
...
on_player_damage( eInflictor, eAttacker, iDamage, iDFlags, sMeansOfDeath, sWeapon, vPoint, vDir, sHitLoc, timeOffset ) 
{
	...
	{
		if( sWeapon == "m320_mp" && isdefined( eInflictor.smoke ) )
		{
			return;
		}

		if( sWeapon == "xm25_mp" && isdefined( eInflictor.pusher ) )
		{
			return;
		}
	}
